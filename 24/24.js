const fs = require('fs');
const input = fs.readFileSync('24/24.txt', 'UTF-8');
...
const p1 = () => {
  ...
  for(const line of lines) {
    const parsed = /(\d+) units each with (\d+) hit points (\([^)]*\) )?with an attack that does (\d+) ([a-z]+) damage at initiative (\d+)/.exec(line);

    if(parsed) {
      const count = +parsed[1];
      const hp = +parsed[2];
      const attackDamage = +parsed[4];
      const attackType = parsed[5];
      const initiative = +parsed[6];
      let immune = [];
      let weak = [];

      if(parsed[3]) {
        const sub = parsed[3].substring(1, parsed[3].length - 2);
  ...
  while(true) {
    units = units.sort((a, b) => {
      const effectiveA = effectivePower(a);
      const effectiveB = effectivePower(b);
    ...
    for(const unit of units) {
      ...
      for(const target of units) {
        if(unit.team !== target.team && claimedTargets.indexOf(target) === -1) {
          ...
          const targetDamage = getDamage(unit, target);
    ...
    for(unit of  units) {
      if(unit.target && unit.count >= 0) {
        const damage = getDamage(unit, unit.target);
        const casualties = Math.floor(damage / unit.target.hp);
